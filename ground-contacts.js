...
import {bindZombieRig} from './zombie-rig.js';
...
export class GroundContacts {
  ...
  update(role,phase,state,scale,weight=1) {
    ...
    for(const side of ['L','R']) {
      const q=((phase+(side==='R'?.5:0))%1+1)%1;
      const planted=this.profile ? q<this.profile.stance[role] : contactPath(role,q).planted;
      const pin=this.pins[side];
      ...
      if(!upper||!lower||!tip||!planted){pin.active=false;continue;}
      ...
      if(this.profile?.id==='caretaker')this.pole.set(sign*(crawl?.61:.13),crawl?.13:.65,crawl?.36:.8);
